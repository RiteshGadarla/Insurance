...
import { ShieldCheck, FileText, Plus, Trash2, CheckCircle2, AlertCircle, Loader2 } from "lucide-react";
import { fetchPolicyById, finalizeInsurancePolicy } from "@/lib/api";

export default function ReviewPolicyRequirementsPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (loading) return (
        <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
            ...
            <div className="text-xl font-medium text-gray-700">AI is analyzing policy document...</div>
            <p className="text-gray-500">Extracting required claim documents and criteria.</p>
        </div>
    );
        ...
        <div className="container mx-auto py-10 max-w-4xl">
            <div className="flex items-center justify-between mb-8">
                ...
                <Button
                    ...
                    disabled={submitting}
                    className="bg-green-600 hover:bg-green-700 text-white gap-2 h-12 px-6 min-w-[200px]"
                >
                    {submitting ? (
                        <>
            ...
            <Card className="shadow-lg border-blue-100">
                <CardHeader className="bg-blue-50/50 border-b">
                    ...
                    <p className="text-sm text-gray-500 font-normal mt-1">
                        Our AI analyzed the policy PDF and identified the following necessary claim documents.
                ...
                <CardContent className="pt-6">
                    <div className="space-y-4">
                        {requiredDocs.map((doc, idx) => (
                            <div key={idx} className="group border rounded-lg p-4 bg-gray-50/50 hover:bg-white hover:border-blue-200 transition-all relative">
                                <button
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-2">
                                        <Label className="text-xs uppercase tracking-wider text-gray-500">Description / Criteria</Label>
                                        <Input
                                            value={doc.description}
                                            onChange={e => updateDocument(idx, "description", e.target.value)}
                                            placeholder="What should this document contain?"
                                            className="bg-white"
                                        />
                                    </div>
                                    <div className="space-y-2 md:col-span-2 flex items-center gap-4">
                                        <div className="flex-1 space-y-2">
                                            <Label className="text-xs uppercase tracking-wider text-gray-500">Notes for Hospital</Label>
                                            <Input
                                                value={doc.notes}
                                                onChange={e => updateDocument(idx, "notes", e.target.value)}
                                                placeholder="Additional hints for the hospital staff"
                                                className="bg-white"
                                            />
                                        </div>
                                        <div className="flex items-center gap-2 self-end pb-2">
                                            ...
                                            />
                                            <Label htmlFor={`mandatory-${idx}`} className="text-sm font-medium cursor-pointer">
                                                Mandatory
                                            </Label>
                                        </div>
                                    </div>
                        ...
                        <Button
                            variant="outline"
                            onClick={addDocument}
                            className="w-full border-dashed border-2 py-8 text-gray-500 hover:text-blue-600 hover:border-blue-300 hover:bg-blue-50 transition-colors gap-2"
                        >
            ...
            <div className="mt-8 flex justify-between items-center bg-gray-50 p-6 rounded-lg border">
                <div className="text-sm text-gray-500">
                    <p>Total suggested requirements: <span className="font-bold text-gray-700">{requiredDocs.length}</span></p>
                    <p>Policy status: <span className="capitalize font-bold text-blue-600">{policy.status}</span></p>
                </div>
                <div className="flex gap-4">
                    <Button variant="ghost" onClick={() => router.push("/insurance/policies")}>
                        Cancel
                    </Button>
                    <Button
                        onClick={handleFinalize}
